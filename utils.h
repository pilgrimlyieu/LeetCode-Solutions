...
ListNode *createListNode(const std::initializer_list<int> &il) { return new ListNode(std::vector<int>(il)); }
...
TreeNode *createBinaryTree(Args &&...args) {
    ...
    auto processArg = [&](auto &&arg) {
        ...
        if (index == 0) {
            ...
            if (parentIdx < nodes.size() && nodes[parentIdx]) {
                if constexpr (std::is_null_pointer_v<T>) {
                    nodes.emplace_back(nullptr);
                }
                else {
                    auto *newNode = new TreeNode(arg);
                    if (index % 2 == 1) {
...
#define BINTREE(...) createBinaryTree(__VA_ARGS__)
